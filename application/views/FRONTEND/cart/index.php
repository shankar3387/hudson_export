...
<div class="container">
   ...
   <table class="table table-bordered">
      ...
      <tbody>
         ...
         <tr id="cartview<?=$row['id']?>">
            <th scope="row"><a style="color:red;" href="<?=base_url('Removecart/').$row['id'];?>" onclick="return confirm('Are You Sure Want To Remove?');"><i class="fa fa-remove"></i></a></th>
            <td><a href="<?=base_url('Product/').$pro_id?>"><img class="img1" src="<?=base_url().$img?>" alt="roadster" width="50" height="50"></a></td>
            ...
            <td class="text-left"><div class="input-group btn-block quantity">
                        ...
                        <?php  if(!empty($row['tbl_quantity'])) { ?>
                        <input type="text" name="quantity" id="quantity<?=$row['id']?>" onkeyup="checkqtystockip(this.value,<?=$pro_id?>)" value="<?php if(!empty($row['tbl_quantity'])) {echo $row['tbl_quantity']; }else{echo '1'; } ?>" size="1" class="form-control" />
                        <span id="qtymess" style="color:red;"></span>
                        
                        <?php }else{ ?>
                        <input type="text" name="quantity" id="quantity<?=$row['id']?>" onkeyup="checkqtystockip(this.value,<?=$pro_id?>)" value="<?php if(!empty($row['tbl_quantity'])) {echo $row['tbl_quantity']; }else{echo '1'; } ?>" readonly size="1" class="form-control" />
                        <span id="qtymess" style="color:red;"></span>
                        
                        <?php } ?>
                        <span class="input-group-btn">
                        <button type="submit" data-toggle="tooltip" title="Update" onclick="updatecart('<?=$row['id']?>')" class="btn btn-primary update"><i class="fa fa-refresh"></i></button>
                        <a href="<?=base_url('Removecart/').$row['id'];?>"  onclick="return confirm('Are You Sure Want To Remove?');"><button type="button" data-toggle="tooltip" title="Remove" class="btn btn-danger" onClick=""><i class="fa fa-times-circle"></i></button></a>
                        </span></div>
                        <?php }else{ ?>
                        <span class="outstock" style="background: #d60c0c;color: #fff;padding: 2px 8px;border-radius: 2px;font-weight: bold;">Out Of Stock</span>
                        <?php }}} ?>
                    </td>
   ...
   <br>
   <center><h2>Cart totals</h2></center>
   <table class="table table-bordered">
      <tbody>
         ...
         <td class="text-right"><?php if($delivery != 0) {echo "<i class='fa fa-usd'></i>".$delivery;  }else{ echo "Free"; } ?></td>
       </tr>
       <tr>
         <td class="text-right"><strong>Total</strong></td>
         <td class="text-right" id="total_price"><i class="fa fa-usd"></i> <?=$totalPrice + $delivery?>
         <input type="hidden" class="totalamnt" name="totalamnt" id="totalamnt" value="<?=$totalPrice + $delivery?>" />
         <input type="hidden" name="newdeductamiunt" id="newdeductamiunt" value="<?=$totalPrice + $delivery?>" />
         </td>
        </tr>
      </tbody>
   </table>
   <a href="<?=base_url('Checkout')?>"><button class="submit pull-right" type="submit" style="border-radius:5px;color: #060606;">Proceed to checkout</button></a>
<?php }else{ ?>
   <div class="alert alert-danger alert-dismissible" id="success-alert">
      <strong><i class="fa fa-shopping-cart"></i></strong> Your cart is currently empty.!!!
   </div>
   <a href="<?=base_url('')?>"><button class="submit pull-left" type="submit" style="border-radius:5px;color: #060606;">Return To shop</button></a>
<?php } ?>
   <br/>
</div>
</br>
